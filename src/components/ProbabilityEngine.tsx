    ...
    <div className="overflow-y-auto scrollbar-thin">
      ...
      <div className="px-6 pt-5 pb-4 border-b border-border">
        <div className="flex items-center justify-between mb-3">
          <div className="text-[10px] tracking-wide text-muted-foreground font-mono flex items-center gap-2">
        ...
        <div className="text-[11px] text-foreground/80 font-medium mb-3 leading-snug line-clamp-2">
          {market?.question || 'Select a market to begin analysis'}
        </div>

        <ConfidenceStrip ci={ci} estimate={pfState.estimate} />

        <div className={`font-display text-[72px] leading-none font-bold tracking-tight transition-colors duration-500 ${probColor}`}>
          {market ? probPercent : '--'}
          <span className="text-[28px] text-muted-foreground ml-1">%</span>
        </div>
        ...
        <div className="grid grid-cols-5 gap-4 mt-4">
          {[
            ...
            <div key={stat.label}>
              <div className="text-[9px] text-muted-foreground tracking-wide mb-0.5 font-mono">{stat.label}</div>
              <div className={`text-[13px] font-mono font-medium ${stat.color || 'text-foreground'}`}>{stat.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 py-4 border-b border-border">
        <div className="text-[9px] tracking-wide text-muted-foreground mb-3 font-mono">
      ...
      <div className="px-6 py-4 border-b border-border">
        ...
        <div className="flex gap-4 mt-2">
          {[
            ...
            <span key={s.label} className="text-[9px] text-muted-foreground font-mono">
